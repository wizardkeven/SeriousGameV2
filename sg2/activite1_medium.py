	...
	def on_touch_up(self, touch):
		...
		if touch.grab_current is self:    
			...
			if (self.category==self.parent.category):
				...
				#Saving in dataBase
				self.parent.local_db.insert_into_Table("activite1_medium",
												["time Date", "score int", "time", "source string", "destination string", "result string" ],
												[time.strftime("%a %d %b %Y %H:%M:%S", time.gmtime()),
												str(val),
												str(tem),
												self.category,
												Activite1_medium.category,
												"Success"
												]
										)
...
class Activite1_medium(Widget):
         
	#Open a dataBase connexion
	local_db = dataBase.DataBase()  
	table_name = "activite1_medium"
	...
	#Elle est appelee dans la methode suivante
		...
		loaded_file = open("./activite1_medium.txt", "r")     
        #read the first line
			...
			if (line[0]!='#'):
				...
				if (tab_res[0]=="Sound"):
					if(tab_res[1]==self.sound):
						Activite1_medium.category =tab_res[2]
			#read the next line
			line = loaded_file.readline()
	...
	#Boutton cree dans le .kv
	def stop(self):
		return Activite1_mediumApp().stop();

class Activite1_mediumApp(App):
